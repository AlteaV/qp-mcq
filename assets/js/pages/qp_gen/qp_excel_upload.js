...
downloadButton.addEventListener("click", () => downloadSampleExcel());
    ...
    if (response.success) {
      ...
      if (response.status === 409) {
        Swal.fire({
          ...
          const tableFormattedData = response.result.duplicate_questions.map(
            (item, index) => ({
              question_no: index + 1,
...
async function showReportSection(data) {
  ...
  data.forEach((record, index) => {
    ...
    let sectionOptions = sectionTopic
      .map((sec) => {
    ...
    let btlField = `
        ...
        ${btlLevel
          .map((level) => {
            return `<option value="${level.level}" ${
              record.btl == level.level ? "selected" : ""
            }>
                ${level.level_name}
            </option>`;
          })
...
function downloadSampleExcel() {
  try {
    const sampleData = [
      ...
      [
        "Discuss the role of equipment like cranes, scaffolding, and derricks in ensuring safety and efficiency during the erection process.",
        "16",
        "3",
      ],
      [
        "Discuss the importance of thermal insulation and fire protection in buildings and Explain the different types of insulation materials.  ",
        "16",
        "1",
      ],
    ];
    ...
    worksheet["!cols"] = [{ wch: 80 }, { wch: 10 }, { wch: 10 }];

    XLSX.utils.book_append_sheet(workbook, worksheet, "Sample");

    const xlsFile = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([xlsFile], { type: "application/octet-stream" });
